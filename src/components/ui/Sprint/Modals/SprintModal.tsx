...
interface SprintModalProps {
    onClose: () => void;
    initialValues?: ICreateSprint & { id?: string };
    mode: "create" | "update";
}
...
export const SprintModal: FC<SprintModalProps> = ({ onClose, initialValues, mode }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await sprintSchema.validate(sprint, { abortEarly: false });
            setErrores({});
            ...
            if (cierre < today) {
                setErrores((prev) => ({
                    ...prev,
                    fechaCierre: "La fecha de cierre no puede ser anterior a hoy",
                }));
                return;
            }
            ...
            if (cierre < inicio) {
                setErrores((prev) => ({
                    ...prev,
                    fechaCierre: "La fecha de cierre no puede ser anterior a la fecha de inicio",
                }));
                return;
            }

            if (isUpdateMode) {
                const actualizado = await updateSprintController(sprint);
                updateSprint(actualizado);
                Swal.fire("Éxito", "Sprint actualizado correctamente", "success");
            } else {
                const creado = await addSprintController({
                    nombre,
                    fechaInicio,
                    fechaCierre,
                    tareas: [],
    ...
    return (
        <div className="overlay">
            <form className={styles.sprintModal_form} onSubmit={handleSubmit}>
                <h3 className={styles.sprintModal_title}>{isUpdateMode ? "Editar Sprint" : "Crear Sprint"}</h3>
