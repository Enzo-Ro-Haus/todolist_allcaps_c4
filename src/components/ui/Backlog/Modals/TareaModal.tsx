...
export const TareaModal: FC<TareaModalProps> = ({
    ...
    useEffect(() => {
        if (isUpdateMode && initialValues) {
            ...
            setFechaLimite(initialValues.fechaLimite);
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await tareaSchema.validate(tarea, { abortEarly: false });
            setErrores({});

            if (isUpdateMode) {
                const result = await updateTareaBacklogController(tarea);
                updateTarea(result);
                clearActiveTarea();
                Swal.fire("Éxito", "Tarea actualizada correctamente", "success");
            } else {
                const tareaCreada = await addTareaBacklogController(tarea);
                if (tareaCreada) {
